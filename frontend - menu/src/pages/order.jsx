...
import Order_S from './order.module.css';
...
export default function Order() {
    ...
    const handleNextClick = () => {
        ...
        navigate('/order'); // Navigate to checkout or any route
    };
    ...
    const totalItems = Object.values(cartItems).reduce((acc, item) => acc + item.count, 0);
    ...
    const renderItems = () => {
        ...
        return filteredItems.map((item, idx) => (
            <div className={Order_S.card} key={idx}>
                <img style={{height:"80px",width:"160px"}} src={item.img} alt={item.title} />

                <div className={Order_S.names}>
                    <h4>{item.title}</h4>

                    <div className={Order_S.bottomRow}>
                        <p className={Order_S.price}>{item.price}</p>

                        {
                            cartItems[item.title] ? (
                                <div className={Order_S.quantityControl}>
                                    <button onClick={() => decrementItem(item)}>-</button>
                                    <span>{cartItems[item.title].count}</span>
                                    <button onClick={() => incrementItem(item)}>+</button>
                                </div>
                            ) : (
                                <p className={Order_S.addButton} onClick={() => handleAddItem(item)}>+</p>
                            )
                        }
                    </div>
                </div>
    ...
    return (
        <div className={Order_S.main}>
            <h1>Good evening</h1>
            <h3>Place your order here</h3>
            <div className={Order_S.searchBar}>
                <span className={Order_S.searchIcon}><Search /></span>
                <input
                    type="text"
                    placeholder="Search"
                    value={searchQuery}
            ...
            <div className={Order_S.tab_container}>
                {/* <div className={Order_S.tabs} onClick={() => setSelectedCategory('burgers')}> */}
                <div
                    className={`${Order_S.tabs} ${selectedCategory === 'burgers' ? Order_S.activeTab : ''}`}
                    onClick={() => setSelectedCategory('burgers')}
                >

                    <Burger className={Order_S.logo} />
                    <p>Burgers</p>
                </div>
                <div className={`${Order_S.tabs} ${selectedCategory === 'pizza' ? Order_S.activeTab : ''}`} onClick={() => setSelectedCategory('pizza')}>
                    <Pizza className={Order_S.logo} />
                    <p>Pizza</p>
                </div>
                <div className={`${Order_S.tabs} ${selectedCategory === 'drinks' ? Order_S.activeTab : ''}`} onClick={() => setSelectedCategory('drinks')}>
                    <Drink className={Order_S.logo} />
                    <p>Drinks</p>
                </div>
                <div className={`${Order_S.tabs} ${selectedCategory === 'fries' ? Order_S.activeTab : ''}`} onClick={() => setSelectedCategory('fries')}>
                    <Fries className={Order_S.logo} />
                    <p>Fries</p>
                </div>
                <div className={`${Order_S.tabs} ${selectedCategory === 'veggies' ? Order_S.activeTab : ''}`} onClick={() => setSelectedCategory('veggies')}>
                    <Veggies className={Order_S.logo} />
                    <p>Veggies</p>
                </div>
            </div>
            <h1>{selectedCategory}</h1>

            <div className={Order_S.cards}>
      ...
      <h1>Total Items: {totalItems}</h1>
            ...
            <button className={Order_S.nextButton} onClick={handleNextClick}>Next</button>
